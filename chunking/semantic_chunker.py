...
class MedicalBoundaryDetector:
    ...
    def __init__(self):
        ...
        self.section_boundaries = {
            ...
            # 症状描述边界
            'symptom': [
                r'主诉', r'现病史', r'既往史', r'体格检查',
                r'辅助检查', r'实验室检查', r'影像学检查'
            ]
        }
    ...
    def detect_boundaries(self, text: str) -> List[int]:
        ...
        # 检测各类型边界
        for boundary_type, patterns in self.section_boundaries.items():
...
class DynamicChunkSizer:
    ...
    def _assess_clinical_importance(self, metadata: Dict[str, Any]) -> float:
        """评估临床重要性"""
        importance = 0.0
...
class SemanticChunker:
    ...
    def __init__(self):
        ...
        self.quality_checker = ChunkQualityChecker()
    ...
    def chunk_document(self, 
        ...
        # 2. 确定分块策略
        content_type = self._infer_content_type(metadata, text)
        ...
        # 5. 质量评估
        self.quality_checker.evaluate_chunks(parent_chunks, child_chunks)
...
class ChunkQualityChecker:
    ...
    def evaluate_chunks(self, parent_chunks: List[Dict], child_chunks: List[Dict]):
        """评估块质量"""
        if not parent_chunks:
            return
        
        # 计算基本统计
        parent_sizes = [len(chunk['page_content']) for chunk in parent_chunks]
        child_sizes = [len(chunk['page_content']) for chunk in child_chunks]
        
        self.metrics['avg_chunk_size'] = np.mean(parent_sizes)
        self.metrics['std_chunk_size'] = np.std(parent_sizes)
        
        # 信息密度评估
        total_words = sum(len(chunk['page_content'].split()) for chunk in parent_chunks)
        total_chars = sum(len(chunk['page_content']) for chunk in parent_chunks)
        self.metrics['information_density'] = total_words / max(1, total_chars / 5)
        ...
        for chunk in parent_chunks:
            ...
            if metadata.get('chunk_type') == 'semantic':
                boundary_quality += 1
        self.metrics['boundary_quality'] = boundary_quality / len(parent_chunks) if parent_chunks else 0
        
        logger.info(f"Chunk quality metrics: {self.metrics}")
...
if __name__ == "__main__":
    # 示例文档
    sample_doc = {
        'page_content': """患者张三，男，65岁，因腰痛伴右下肢放射痛3月入院。2024-02-10行L4/5椎间盘切除术。
术后第1天：卧床休息，轴向翻身。术后第3天：开始床上踝泵运动。术后第7天：在支具保护下坐起。
诊断：L4/5椎间盘突出症。注意事项：避免剧烈运动，定期复查MRI。
""",
        'metadata': {
            'title': '腰椎间盘突出症术后康复指南',
            'category': '骨科手术',
    ...
    parents, children = chunker.chunk_document(sample_doc)
    
    print(f"生成 {len(parents)} 个父块，{len(children)} 个子块")
    for i, parent in enumerate(parents):
        print(f"Parent {i+1}: {len(parent['page_content'])} chars, type: {parent['metadata'].get('chunk_type')}")